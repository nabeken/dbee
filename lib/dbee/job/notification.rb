  ...
  module Job
    class Notification
      ...
      def self.perform(request_id, running_job, args)
        ...
        # ダウンロードの平均速度 (MB/s)
        download_speed = sprintf("%s.3f", (download_size / (download_finished_at - download_started_at)) / (1000.0 ** 2))
        ...
        # ダウンロードの平均速度 (MB/s)
        upload_speed = sprintf("%s.3f", (upload_size / (upload_finished_at - upload_started_at)) / (1000.0 ** 2))
